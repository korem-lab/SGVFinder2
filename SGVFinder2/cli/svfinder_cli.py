import logging
import argparse
from ..svfinder import work_on_collection
...
parser.add_argument('--samp_to_map_dir', help='Path to folder with `.smp` files')
...
parser.add_argument('--max_spacing', help='Max spacing param (default 10)', type=float, default=10)
...
parser.add_argument('--min_samp_cutoff', help='Minimum number of samples in which a microbe exists with sufficient coverage to be considered in the analysis (Default=2)',type=int, default=2)
...
parser.add_argument('--taxonomy_path',
                    ...
                    type=str, default=None)
parser.add_argument('--genepos_path',
                    help='Optional; gene position path (must end in `.genepos.df`)',
                    type=str, default=None)
parser.add_argument('--bac_frames_path',
