...
class ReadContainer(object):
    ...
    def add_pmp_sr(self, sr, pe, allowed_dests, max_mismatch, filtermaps, dest_dict):
        cur = []
        if pe is None or pe:
        ...
        if len(cur) == 1:
            ...
            for mp, newdest, addpos in cur:
                qual = _calc_qual(mp)
                cur2.append((qual, addpos + (mp.pos if hasattr(mp, 'pos') else mp.pos1),
                             -1 if hasattr(mp, 'pos') else (addpos + mp.pos2), self._id_from_dest(newdest)))
    ...
    def get_full_delta(self, pi, theta1, epsilon, use_theta):
        ...
        for rrid, mpngs in self._ambig_rds.items():
            if use_theta:
                noms = tuple(
                    (self._did_to_dest[did], pos1, pos2, pi[did] * theta1[did] * qual, qual) for qual, pos1, pos2, did
                    in mpngs)
            else:
                noms = tuple(
                    (self._did_to_dest[did], pos1, pos2, pi[did] * qual, qual) for qual, pos1, pos2, did in mpngs)
...
def _get_flank_size(length, numbins, avg_read_length):
    if length > avg_read_length * 10:
        return avg_read_length
    else:
        return max(75, length / numbins)  # TODO: parametrize 75


def _add_read(covmap, pos1, pos2, avg_read_length, koef, length):
    numbins = len(covmap)
    flank_size = _get_flank_size(length, numbins, avg_read_length)
    binlength = (length - (flank_size * 2.)) / (numbins - 2)
    ind1 = int((pos1 / binlength) - (flank_size / binlength) + 1)
    ind2 = int((pos2 / binlength) - (flank_size / binlength) + 1)
    pos1_end = pos1 + avg_read_length
    pos2_end = pos2 + avg_read_length
    ind1_end = int((pos1_end / binlength) - (flank_size / binlength) + 1)  ##TODO: Need to adjust better for PE
    ind2_end = int((pos2_end / binlength) - (flank_size / binlength) + 1)
    ...
    if pos2 > -1:  # PE
        ...
        if ind2 < ind2_end:
            if 0 < ind2 < numbins - 1:
                covmap[ind2] += koef * ((binlength - ((pos2 - flank_size) % binlength)) / avg_read_length)
            if ind2_end < numbins - 1:
                covmap[ind2_end] += koef * (((pos2_end - flank_size) % binlength) / avg_read_length)
    ...
    if ind1 < ind1_end:
        if 0 < ind1 < numbins - 1:
            covmap[ind1] += koef * ((binlength - ((pos1 - flank_size) % binlength)) / avg_read_length)
        if ind1_end < numbins - 1:
            covmap[ind1_end] += koef * (((pos1_end - flank_size) % binlength) / avg_read_length)
...
def _calc_qual(pmap_map):
    ret_val = 0
    if hasattr(pmap_map, 'map_quality'):
        ret_val = 10 ** pmap_map.map_quality
    elif hasattr(pmap_map, 'map_quality1'):
        ret_val = 10 ** (pmap_map.map_quality1 + pmap_map.map_quality2)
    else:
