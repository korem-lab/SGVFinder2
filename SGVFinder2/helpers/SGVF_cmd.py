...
def _addargs(parser):
    ...
    parser.add_argument('output_dsgv',
                        help='Output path for the deletion-sgv dataframe. By default a pickled pandas dataframe')
    parser.add_argument('output_vsgv',
                        help='Output path for the variable-sgv dataframe. By default a pickled pandas dataframe')
    parser.add_argument('--x_coverage',
                        help='The desired coverage across the genome in units of 100bp reads. This parameter is used '
                             'to determine bin size: bin_size = rate_param/x_coverage (Default = 0.1)',
                        type=float, default=0.1)
    parser.add_argument('--rate_param',
                        help='The lower limit for the median number of reads per genomic bin. Genomes with coverage '
                             'lower than rate_param will be discarded from the analysis (Default = 10)',
                        type=int, default=10)
    parser.add_argument('--byorig', help='Calculate SGVs according to the regions published by Zeevi et al, XXX, 201Y.',
                        action='store_true')
    parser.add_argument('--min_samp_cutoff',
                        help='Minimum number of samples in which a microbe exists with sufficient coverage to be '
                             'considered in the analysis (Default=75)',
                        type=int, default=75)
    parser.add_argument('--dels_detect_thresh',
                        help='Determines the minimum and maximum ratio of samples for which a bin is considered a '
                             'deletion-SGV. (Default=0.25, setting the ratio at 0.25-0.75)',
                        type=float, default=0.25)
    parser.add_argument('--real_del_thresh',
                        help='Threshold above which a bin is considered deleted for all individuals (Default=0.95)',
                        type=float, default=0.95)
    parser.add_argument('--vsgv_dissim_thresh',
    ...
    parser.add_argument('--dels_cooc_thresh',
                        ...
                        type=float, default=0.25)
    parser.add_argument('--vsgv_clip_quantile',
    ...
    parser.add_argument('--vsgv_dense_perc',
    ...
    parser.add_argument('--browser_path',
                        help='Optional; a path for the html output of SGV-Browser (Default=None, resulting in no '
                             'output)',
...
if __name__ == '__main__':
    ...
    if args.byother:
        vsgv, dsgv = calculate_by_other(join(split(realpath(__file__))[0], '../DataFiles/orig_dsgv.df'),
                                        ...
                                        samp_to_map, args.real_del_thresh, args.vsgv_dense_perc, args.browser_path,
                                        args.x_coverage, args.rate_param)
    else:
        vsgv, dsgv = work_on_collection(samp_to_map, args.min_samp_cutoff, args.dels_detect_thresh,
                                        args.real_del_thresh,
                                        args.dels_cooc_thresh, args.vsgv_dissim_thresh, args.vsgv_clip_quantile,
                                        args.vsgv_fit_interval,
                                        args.vsgv_fit_method, args.x_coverage, args.rate_param, args.vsgv_dense_perc,
                                        args.browser_path)
    if args.csv_output:
