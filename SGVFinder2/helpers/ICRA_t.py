...
def single_file(fq1, fq2, outfol, max_mismatch, consider_lengths, epsilon,
                ...
                length_maximum=1e6, dbpath=None, use_theta=False, threads=1, senspreset, report_alns, max_ins):
    log_.debug('Loading database...')
    seqdict, indexf, dest_dictf = load_db(dbpath)
    outpref = join(outfol, basename(fq1).replace('_1.fastq', '').replace('.fastq', ''))
    ...
    read_container, pi, theta1, average_read_length, lengthdb = \
        _initialize(fq1, fq2, outpref + '.pmp', seqdict, max_mismatch, consider_lengths, length_minimum, length_maximum,
...
def _calc_qual(sr, pmap_map, seqdict):
    if hasattr(pmap_map, 'mismatches'):
        ga1, off1 = _get_opt_gigar_array(seqdict, pmap_map.dest_id, pmap_map.strand, pmap_map.pos, pmap_map.mismatches,
                                         sr.seq[pmap_map.end], pmap_map.gigar)
        return 10 ** gqa.calc_quality(sr.quals[pmap_map.end], ga1), off1, None
    else:
        ga1, off1 = _get_opt_gigar_array(seqdict, pmap_map.dest_id, pmap_map.strand1, pmap_map.pos1,
                                         pmap_map.mismatches1, sr.seq[0], pmap_map.gigar1)
        ga2, off2 = _get_opt_gigar_array(seqdict, pmap_map.dest_id, pmap_map.strand2, pmap_map.pos2,
                                         pmap_map.mismatches2, sr.seq[1], pmap_map.gigar2)
...
def load_db(db_prefix):
    assert exists(db_prefix + '.if_ujsn'), 'Could not find database file at ' + db_prefix + '.if_ujsn'
    return CSeqDict.CSeqDict(db_prefix + '.if_ujsn', 18e9), db_prefix + '.btindex', db_prefix + '.dests'
